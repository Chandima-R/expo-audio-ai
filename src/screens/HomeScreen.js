    ...
    // //first need to copy the ip from ipconfig(wlan one) and paste it in the axios url
    // //change the port number to 5000 with the ip address
...
export default function HomeScreen() {
    ...
    const [isRecording, setIsRecording] = useState(false);
    const [recordedAudioData, setRecordedAudioData] = useState(null); // Add this state variable
    const [convertedAudioData, setConvertedAudioData] = useState(null); // Add this state variable
    ...
    async function stopRecording() {
        if (recording) {
            ...
            try {
                ...
                const wavFile = await convertToWav(audioData);

                // Set the converted WAV audio data in the state
                setConvertedAudioData(wavFile);

                // Send the converted audio, selectedLanguage, and selectedGrade to the API
                await sendAudioToAPI(wavFile, selectedLanguage, selectedGrade);
            } catch (err) {
                console.error('Failed to stop recording', err);
            }
        }
    ...
    async function convertToWav(sourceUri) {
        const outputUri = `${FileSystem.documentDirectory}output.wav`;
        const { sound } = await Audio.Sound.createAsync({ uri: sourceUri });
        await sound.downloadAsync();
        await sound.setURI(outputUri);
        ...
        <SafeAreaView style={styles.container}>
            ...
            {convertedAudioData && (
                <View style={styles.row}>
                    <Text style={styles.fill}>Converted WAV Audio</Text>
                    <TouchableOpacity
                        style={styles.messageListButton}
                        onPress={() => {
                            // Play the converted audio if needed
                            // You can use Expo AV to play it similar to how you play other recordings
                        }}
                    >
                        <Text style={styles.messageListButtonText}>Play</Text>
                    </TouchableOpacity>
                </View>
            )}
